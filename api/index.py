...
def perform_gemini_multimodal_analysis(media_urls: List[str], caption: str, media_type: str, knowledge: str) -> Dict[str, Any]:
    ...
    以下の「マニュアル」に記載されている施策【のみ】を基準として、ユーザーの投稿を添削してください。
    
    ## 重要な制約
    - 分析・フィードバックは必ず「マニュアル」に記載されている施策のみを根拠としてください。
    - マニュアルには「▶〇〇を上げる施策」などの形式で各施策が記載されており、その中に「・〜」という箇条書きで具体的な内容が書かれています。
    - 一般的なInstagram運用知識ではなく、必ずマニュアル内の具体的な項目を引用して指摘してください。
    - フィードバックには、参照したマニュアルの施策名（▶で始まる項目名）を明記してください。
    ...
    ## 指示
    1. {"カルーセル投稿の全画像の内容を確認し、1枚目から最後までのストーリー性、情報の配置、テロップの一貫性を分析してください。" if is_carousel else "動画/画像の内容を分析してください。"}
    2. マニュアルに記載されている施策に照らし合わせて、100点満点でのスコアリングを行ってください。
    ...
    5. 各フィードバックの suggestion フィールドには、必ず「マニュアルのどの施策（▶〇〇）に基づく指摘か」を明記してください。
    6. 明日から実行できる3つの具体的なアクションプランを提示してください（これもマニュアルの施策に基づくこと）。
    ...
    interface FeedbackItem {{
      ...
      point: string;      // 指摘内容
      suggestion: string; // 改善案。必ず「【▶〇〇】」の形式でマニュアルの施策名を先頭に記載すること
    }}
    ...
    interface AnalysisResponse {{
      ...
      action_plan: string[]; // 具体的なネクストアクション3つ。各項目の先頭に「【▶〇〇】」の形式でマニュアルの施策名を記載すること
    }}
