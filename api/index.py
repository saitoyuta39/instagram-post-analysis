...
def perform_gemini_multimodal_analysis(media_urls: List[str], caption: str, media_type: str, knowledge: str) -> Dict[str, Any]:
    ...
    3. **分析プロセス**:
        ...
        - 次に、投稿内容（画像、動画、および【キャプション本文】）を細部まで観察してください。
    ...
    ## 出力形式の指示
    1. 投稿の【画像/動画】と【キャプション本文】の両方を深く分析してください。
       - 画像/動画: 情報の配置、テロップ、デザイン、ストーリー性など。
       - キャプション: 冒頭の引き（フック）、読みやすさ、CTA（保存・フォロー誘導）、ハッシュタグの適切性など。
    ...
    interface FeedbackItem {{
      ...
      location?: string;  // 例: "1枚目", "キャプション全体", "ハッシュタグ部分"
      point: string;      // 指標名。例: "【▶保存率を上げる施策】キャプションでの保存誘導"
